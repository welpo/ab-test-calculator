...
function calculateSampleSize(
  ...
  const treatmentCR = baselineCR + absoluteMDE;
  ...
  const zAlpha =
    testType === "one-sided"
      ? normSInv(1.0 - adjustedAlpha) // one-tailed test.
      : normSInv(1.0 - adjustedAlpha / 2.0); // two-tailed.
  ...
  const varControl = clippedBaselineCR * (1.0 - clippedBaselineCR);
  const varTreatment = clippedTreatmentCR * (1.0 - clippedTreatmentCR);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateTabUI(selectedTabId) {
    tabSingleLabel.setAttribute(
      "aria-selected",
      selectedTabId === "tab-single"
    );
  ...
  function updateMDEMode() {
    ...
    if (isRelativeMode) {
      mdeLabel.textContent = "Minimum detectable effect (%)";
    } else {
      mdeLabel.textContent = "Minimum detectable effect (points)";
    }
    // Update table headers.
    const tableMDEHeader = document.querySelector("#mdeTable th:first-child");
    const tableConversionHeader = document.querySelector(
      "#mdeTable th:nth-child(2)"
    );

    if (tableMDEHeader && tableConversionHeader) {
      if (isRelativeMode) {
        tableMDEHeader.innerHTML = "MDE (%)";
      } else {
        tableMDEHeader.innerHTML = "MDE (points)";
      }
    }
    updateCalculation();
    updateMDETooltips();
  ...
  function updateMDETooltips() {
    ...
    if (absoluteTooltip) {
      absoluteTooltip.textContent = `'${mde}' means going from ${baseline}% to ${absoluteIncrease}% conversion rate`;
    }
  }
  ...
  function updateVariantDistributionUI(variantCount) {
    ...
    const resetDistributionBtn = document.getElementById(
      "resetDistributionBtn"
    );
  ...
  function getVariantDistributions(variantCount) {
    ...
    return {
      distributions:
        variantDistributions.length === variantCount
  ...
  function updateResultsDisplay(results) {
    durationValueElem.textContent = Math.round(
      results.duration
    ).toLocaleString();
  ...
  function addTableRow(
    ...
    if (inputClass === "mde-input") {
      newRow.innerHTML = `
            <td class="editable">
                <input type="number" class="${inputClass}" value="${defaultValue}" step="0.1" aria-label="MDE value">
            </td>
  ...
  function encodeParametersToURL() {
    ...
    for (let i = 0; i < parseInt(variantsSelect.value); i++) {
      ...
      if (slider) {
        ...
        if (
          i > 0 &&
          slider.value !== document.getElementById("sliderA").value
        ) {
          isUneven = true;
        }
      }
